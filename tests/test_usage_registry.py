...
# Prefer providers/*.usage_collect when registered — not the local fallbacks.
...
# Every registered provider with a usage_source must appear.
...
def fake_collect(base: Path) -> list[u.UsageRow]:
    ...
    # Touch base so we prove collect() handed us the projects_dir.
    _ = base.is_dir()
    return [row]


# Ensure clean slate if a previous run left the fake id around.
...
try:
    ...
    # New harnesses appear only after refresh — mirrors post-import registration.
    u.refresh_usage_adapters()

    assert_eq(
        "fake source appears in ADAPTERS after refresh",
        True,
        FAKE_SOURCE in u.ADAPTERS,
    )
    assert_eq(
    ...
    # Existing registry sources still present and not the local fallbacks.
    assert_eq(
    ...
    assert_eq(
        "fake collect: input_tokens from injected collector", 42, agg_fake["totals"]["input_tokens"]
    )
    assert_eq(
    ...
    # Catalog subprocess is a fresh process — won't see in-process fake.
    # In-process list_providers must include it.
    live_sources = {p.usage_source for p in list_providers() if p.usage_source and p.usage_collect}
    assert_eq(
    ...
    pb._REGISTRY.pop(FAKE_PROVIDER_ID, None)
    u.refresh_usage_adapters()
    # Cleanup must remove the fake source from ADAPTERS.
