...
NO network calls. Stdlib-only runner (pytest-less PASS/FAIL pattern).
Run:  python3 tests/test_package_interfaces.py
Called by tests/run-tests.sh via relay_python.
"""
...
# --- routing.resolve: no backends/chats → legacy ----------------------------
...
eq(
    "resolve empty cfg as_pipe",
    "||hello there|legacy",
    r_legacy.as_pipe(),
)

# --- routing.resolve: sticky project-only + @grok prefix --------------------
cfg_sticky = {
    "backends": {
...
# --- routing.resolve: default_backend when no prefix ------------------------
cfg_default = {
    "backends": {
...
# --- hooks.dispatch_hook OK -------------------------------------------------
status_ok, body_ok = hooks.dispatch_hook(
    "grok",
...
# --- hooks.dispatch_hook Stop disabled → SKIP -------------------------------
...
# --- metrics.emit_metric writes temp .metrics.log ---------------------------
...
# --- tts.strip_formatting removes code and URLs -----------------------------
