...
def main() -> int:
    ...
    rows = [
        ...
        u.UsageRow(NOW - 999999, "anthropic", "claude-opus-4-8", "p", 1, 1, 0, 0),
    ]
    ...
    assert_eq(
        "aggregate: by_provider keys", {"anthropic", "openai"}, set(agg["by_provider"].keys())
    )
    assert_eq(
        "aggregate: by_provider anthropic total",
        165,
    ...
    assert_eq(
        "collect: total_events (window-filtered + nested subagent rows)", 7, agg["total_events"]
    )
    assert_eq(
    ...
    assert_eq(
        "collect: never raises, no 'skipped' reason for a good source", None, agg.get("skipped")
    )
    ...
    assert_eq(
        "collect: unknown adapter -> total_events 0 (never fabricated)",
        0,
        unknown_agg["total_events"],
    )
    with tempfile.TemporaryDirectory() as tmpdir:
        missing_agg = u.collect(
            "claude-code", str(Path(tmpdir) / "does-not-exist"), WINDOW, now=NOW
        )
    ...
    if "display disabled" in toggled_text and "claude-opus-4-8" not in toggled_text:
        ok(
            "_render_usage_text(): providers/models=False labels the section disabled, omits the data"
        )
    else:
        fail(
            "_render_usage_text(): providers/models=False labels the section disabled", toggled_text
        )
