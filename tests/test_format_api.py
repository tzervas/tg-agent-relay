...
Run:
  uv run pytest tests/test_format_api.py
...
def main() -> int:
    ...
    eq("wrap: line already <= wrap_width is untouched", [SHORT], wrap_line(SHORT, 50))
    LONG = "this is a much longer line of plain prose that will need to be wrapped at word boundaries to fit a phone screen nicely"
    ...
    true(
        ...
        any(
            "https://example.com/a/very/long/path/that/would/never/fit/in/fifty/characters/at/all"
            in ln
            for ln in url_w
        ),
        repr(url_w),
    ...
    eq("header: leading-emoji ALL-CAPS short line -> bolded", "<b>✅ BUILD FINISHED</b>", r.text)
    r = fmt("🚀 Deploy Started")
    eq("header: leading-emoji Title-Case short line -> bolded", "<b>🚀 Deploy Started</b>", r.text)
    r = fmt("✅ code-reviewer finished — Found 2 issues, both low severity")
    true("header: leading-emoji SENTENCE (lowercase) is NOT bolded", "<b>" not in r.text, r.text)
    ...
    RUST_LONG = "fn this_is_a_very_long_rust_line_that_would_definitely_exceed_the_fifty_char_wrap_width(x: i32) -> i32 { x }"
    r = fmt(f"before\n\n```rust\n{RUST_LONG}\n```\n\nafter")
    eq(
    ...
    eq("unrecognized language tag → plain <pre> (no class)", "<pre>some content</pre>", r.text)
    ...
    eq("inline code: backticks → <code>", "run <code>foo bar</code> now", r.text)
    ...
    with (
        ...
        r = fmt(src)
        eq("never-silent: balance failure → escaped plain text", escape_html(src), r.text)
    ...
    eq("render_code_block empty lang → plain pre", "<pre>hi</pre>", render_code_block("", "hi"))
