...
import tts_plain_text as mod  # noqa: E402
...
WS = strip("a\n\n\nb    c")
check("whitespace collapses (no double spaces / newlines)", "\n" not in WS and "  " not in WS, repr(WS))
...
proc = subprocess.run(
    [sys.executable, str(REPO_ROOT / "lib" / "tts_plain_text.py"),
     "--code-ref", "CLICODE", "--link-ref", "CLILINK"],
    input="## H\n`c` [t](http://u)",
    capture_output=True,
    text=True,
)
