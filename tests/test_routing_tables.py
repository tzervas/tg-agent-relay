...
def main() -> int:
    ...
    CFG_MULTI = {
        ...
        "backends": {
            "claude": {"tag": "claude", "prefixes": ["@claude", "claude:"], "project": "mycelium"},
            "grok": {"tag": "grok", "prefixes": ["@grok"], "project": "mycelium"},
        },
        "chats": [
    ...
    CFG_PROJECT_ONLY = {
        ...
        "projects": {"mycelium": {"root": "/tmp/mycelium-proj", "default_backend": "claude"}},
        "chats": [{"chat_id": -100999, "thread_id": 3, "project": "mycelium"}],
    }
    RESOLVE_PIPE_CASES: list[tuple[str, dict, str, str, str, str]] = [
        ("no backends/chats → legacy", {}, "1", "", "hello", "||hello|legacy"),
        (
            "prefix @grok",
            {
        ...
        (
            "longest prefix wins",
            {"backends": {"short": {"prefixes": ["@g"]}, "long": {"prefixes": ["@grok"]}}},
            "1",
            "",
            "@grok yes",
            "long||yes|prefix",
        ...
        ("sticky other chat", CFG_MULTI, "-1002", "", "hello", "grok|mycelium|hello|chat"),
        (
    ...
    CFG_LOOKUP = {
        "chats": [
            ...
            {"chat_id": -2, "project": "mycelium"},
            {"chat_id": -3, "backend": "grok", "project": "other"},
            {"chat_id": -4, "backend": "claude"},
        ]
    }
    eq(
    ...
    eq(
        ...
        ("-4", ""),
        routing.lookup_chat({"chats": [{"chat_id": -4, "backend": "claude"}]}, "claude", ""),
    )
    eq("lookup_chat miss", ("", ""), routing.lookup_chat(CFG_LOOKUP, "unknown", "nope"))
    hit = routing.strip_prefix(
        {"backends": {"grok": {"prefixes": ["@grok"], "project": "p"}}}, "@grok hi"
    )
    eq("strip_prefix tuple", ("grok", "p", "hi"), hit)
    eq(
        "chat_binding hit",
        True,
        routing.chat_binding({"chats": [{"chat_id": -1, "project": "p"}]}, "-1", "") is not None,
    )
    eq("chat_binding miss", None, routing.chat_binding({"chats": [{"chat_id": -1}]}, "999", ""))
    eq(
    ...
    eq("format_tag empty backend", "", routing.format_tag({}, "", "x"))
    eq(
        "inbound_tag both",
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        (chats_d / "bindings.json").write_text(
            json.dumps(
                {
                    "chats": [
                        {"chat_id": -100, "backend": "grok", "project": "from-overlay"},
                        {"chat_id": -200, "project": "sticky-only"},
                    ]
                }
            ),
            encoding="utf-8",
