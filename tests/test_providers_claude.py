...
def main() -> int:
    ...
    sub = format_hook(
        {"agent_type": "Explore", "last_assistant_message": "found 3 files\nwith matches"},
        "SubagentStop",
        {"prefix": "✅"},
    )
    if "✅" in sub and "Explore" in sub and ("found 3 files" in sub) and ("\n" not in sub):
    ...
    proc2 = subprocess.run(
        [sys.executable, str(REPO / "lib" / "provider_hook.py"), "claude", "--config-json", cfg],
...
def test_providers_claude() -> None:
