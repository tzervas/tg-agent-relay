...
def main() -> int:
    ...
    if "model-turns avoided: 4" in stats_text and "Declared estimate" in stats_text:
        ...
        fail(
            "render_text_stats includes the honest model-turns-avoided line + its caveat",
            stats_text,
        )
    ...
    cli_stats = subprocess.run(
        [
            sys.executable,
            str(REPO_ROOT / "lib" / "metrics_agg.py"),
            str(FIXTURE),
            "9999999",
            "stats",
        ],
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        rc = dashboard_render.main(
            ["dashboard_render.py", str(FIXTURE), str(WINDOW_HOURS), out_png]
        )
