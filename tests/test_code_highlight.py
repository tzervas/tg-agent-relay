...
def main() -> int:
    ...
    assert_eq(
        "_parse_args: a malformed --max-lines= is ignored, default kept", 60, opts3["max_lines"]
    )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert_eq(
            "a block over max_lines -> graceful skip (never an unbounded document)", False, ok_
        )
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert_eq(
                "pygments unavailable (mocked ImportError) -> graceful skip, never a crash",
                False,
                ok_,
            )
            assert_true(
    ...
    print(
        "== lib/code_highlight.py: MyceliumLexer token coverage (Declared, lexical approximation) =="
    )
    import importlib.util

    _HAS_PYGMENTS = importlib.util.find_spec("pygments") is not None
    ...
    if _HAS_PYGMENTS:
        ...
        tokens = [
            (str(kind), text) for kind, text in myc_lexer.get_tokens(MYC_SAMPLE) if text.strip()
        ]
        kind_by_text = {}
        ...
        assert_eq(
            "_get_lexer(unknown tag) falls back to TextLexer", "Text only", unknown_lexer.name
        )
    ...
    if _HAS_PYGMENTS:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert_eq(
                "adversarial lang/title still renders successfully (never-raise contract)",
                True,
                ok_,
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert_eq(
                "unknown-language block still renders (plain-text lexer, still a document)",
                True,
                ok_,
            )
            assert_true(
