...
chmod +x "$DEST"/*.sh "$DEST"/adapters/*.sh "$DEST"/handlers/*.sh \
    "$DEST"/lib/*.sh "$DEST"/scripts/*.sh 2>/dev/null || true

# Python import path: prefer editable install into DEST/.venv; always write env helpers.
install_deploy_python_env() {
    local dest="$1"
    printf '%s\n' "$dest" > "$dest/.pythonpath"
    {
        printf '# Generated by deploy-local.sh — source from shell or systemd units.\n'
        printf 'BRIDGE_DIR=%q\n' "$dest"
        printf 'export BRIDGE_DIR\n'
        printf 'export PYTHONPATH=%q\n' "$dest/lib:$dest"
        if [[ -x "$dest/.venv/bin/python" ]]; then
            # shellcheck disable=SC2016
            printf 'export PATH="%s/.venv/bin:$PATH"\n' "$dest"
            printf 'export RELAY_PYTHON="%s/.venv/bin/python"\n' "$dest"
        fi
    } > "$dest/env.sh"
    ...
    printf 'deploy-local.sh: PYTHONPATH fallback via lib/exec-env.sh and %s/env.sh\n' "$dest"
}

install_deploy_python_env "$DEST"
