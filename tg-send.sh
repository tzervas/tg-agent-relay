...
# on real failure (stderr + metrics), sticky under outage (no re-probe storms),
# secret-redacted reasons. See lib/python_fallback.sh.
_PY_SEND_FALLBACK_REASON=""
_PY_SEND_FALLBACK_KIND=""  # failed | forced | sticky
# shellcheck disable=SC1091
[[ -f "$BRIDGE_DIR/lib/python_fallback.sh" ]] && source "$BRIDGE_DIR/lib/python_fallback.sh"
if declare -f relay_py_try_default >/dev/null 2>&1; then
    # execs on success; returns 1 → shell body below
    relay_py_try_default send tg_agent_relay.send "$@" || true
    _PY_SEND_FALLBACK_KIND="${RELAY_PY_FALLBACK_KIND:-}"
    _PY_SEND_FALLBACK_REASON="${RELAY_PY_FALLBACK_REASON:-}"
else
    # Minimal legacy probe if fallback lib missing (degraded install)
    if [[ "${RELAY_PYTHON_SEND:-1}" == "0" ]]; then
        ...
        if [[ -z "$_py_bin" ]]; then
            _PY_SEND_RC=127
            _PY_SEND_ERR="no Python interpreter found"
        else
            _PY_SEND_ERR="$("$_py_bin" -c "import tg_agent_relay.send" 2>&1)" || _PY_SEND_RC=$?
        fi
        if [[ "$_PY_SEND_RC" -eq 0 ]]; then
            exec "$_py_bin" -m tg_agent_relay.send "$@"
        fi
        _PY_SEND_FALLBACK_KIND="failed"
        _PY_SEND_FALLBACK_REASON="import failed (rc=${_PY_SEND_RC}, bin=${_py_bin:-none})"
    fi
fi
...
if [[ -n "${_PY_SEND_FALLBACK_KIND:-}" ]]; then
    if declare -f relay_py_announce_fallback >/dev/null 2>&1; then
        relay_py_announce_fallback "tg-send" "$_PY_SEND_FALLBACK_KIND" "$_PY_SEND_FALLBACK_REASON"
    else
        if [[ "${RELAY_PYTHON_FALLBACK_QUIET:-0}" != "1" ]]; then
            printf 'tg-send: Python path skipped (%s) — shell recovery.\n' "$_PY_SEND_FALLBACK_REASON" >&2
        fi
